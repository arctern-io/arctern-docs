...
> **注意：** 你需要将 `</path/to/python>` 替换为 Arctern 所在的 Conda 环境内 Python 的绝对路径，此 Python 路径可通过 which python 命令查询。你需要将 `</path/to/spark>` 替换为 Arctern 所在的 Conda 环境内 Spark 的绝对路径。
